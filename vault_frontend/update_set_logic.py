...
class UpdateSetLogic:
    ...
    def update_menu_owned(self):
        ...
        try:
            if update_choice == '1':
                new_name = Validator500000000.get_set_name(
                    "Enter the new name of the set: ")
                ...
                new_theme = Validator500000000.get_theme(
                    "Enter the new theme of the set: ")
                self.vault_manager.update_set_theme(self.set_num, new_theme)
                update_split_files(self.vault_manager)
                print("Lego set theme updated successfully!")

            elif update_choice == '3':
                new_year = Validator500000000.get_year(
                    "Enter the new year of the set: ")
                self.vault_manager.update_set_year(self.set_num, new_year)
                update_split_files(self.vault_manager)
                print("Lego set year updated successfully!")

            elif update_choice == '4':
                new_price = Validator500000000.get_price(
                    "Enter the new price of the set: ")
                self.vault_manager.update_set_price(self.set_num, new_price)
                update_split_files(self.vault_manager)
                print("Lego set price updated successfully!")

            elif update_choice == '5':
                new_piece_count = Validator500000000.get_pieces(
                    "Enter the new piece count of the set: ")
                self.vault_manager.update_set_pieces(self.set_num,
                                                     new_piece_count)
                update_split_files(self.vault_manager)
                print("Lego set piece count updated successfully!")

            elif update_choice == '6':
                new_status = Validator500000000.get_status(
                    "Enter the new status of the set: ")
    ...
    def update_menu_wanted(self):
        ...
        try:
            if update_choice == '1':
                ...
                new_priority = Validator500000000.get_priority(
                    "Enter the new priority of the set: ")
                self.vault_manager.update_set_priority(self.set_num,
                                                       new_priority)
                update_split_files(self.vault_manager)
                print("Lego set priority updated successfully!")
