...
class SetNameInput(ValidateInput):
    ...
    def validate(value):
        ...
        if not stripped_set.isalnum():
            raise SetNameError(
                f"You typed {set} which is not a name formatted correctly (Ferrari F2004 & Michael Schumacher). Please try again."
            )
...
class ThemeInput(ValidateInput):
    ...
    def validate(value):
        ...
        if not stripped_theme.isalnum():
            raise ThemeError(
                f"You typed {theme} which is not a theme formatted correctly (Star Wars). Please try again."
            )
            ...
            try:
                if value.lower() in accepted:
                    ...
                    if int(value) < 1 or int(value) > max_option:
                        raise MenuError(
                            f"You typed {value} which is not a valid option (1-{max_option}). Please try again."
                        )
                    return value
                else:
                    raise MenuError(
                        f"You typed '{value}' which is not a valid option. Please try again."
                    )
